...
if [ "${3:-}" != "" ]; then
  ...
  if [ "${VIDEO_FORCE:-0}" != "1" ] && [ -f "$OUT/${NAME}.mp4" ]; then
    echo "[clip] already have $OUT/${NAME}.mp4 (skip; set VIDEO_FORCE=1 to redo)"; exit 0
  fi
...
FORCE="${VIDEO_FORCE:-0}" python3 - "$MP4" "$OUT" "$JSON" <<'PY'
...
for i, c in enumerate(clips, 1):
    ...
    if not force and os.path.exists(fn):
        print(f"[clip] already have {fn} (skip; set VIDEO_FORCE=1 to redo)")
        continue
